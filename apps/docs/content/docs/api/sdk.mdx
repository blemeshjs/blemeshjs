...
| --- | --- | --- |
| `bindAllEvents` | `(handler: Partial<E>) => () => void` | No description yet. |
| `emit` | `(event: K, args: Parameters<NetworkConnectionEvents[K]>) => void` | No description yet. |
...
| `off` | `(event: K, fn?: NetworkConnectionEvents[K]) => void` | No description yet. |
| `on` | `(event: K, fn: NetworkConnectionEvents[K]) => () => void` | No description yet. |
...
**Source:** `packages/sdk/src/mesh-network/provision.ts#L116`
